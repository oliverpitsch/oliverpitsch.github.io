  ...
  return (
    <Link
      ...
      className={[
        ...
        'border border-[#E2E8F0] dark:border-[#283B63] p-2',
        'bg-white dark:bg-[#152544]',
        'shadow-sm ring-0 transition hover:ring-4 ring-amber-400/50',
        'transition-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500',
        'focus-visible:ring-offset-2 focus-visible:ring-offset-[#F8FAFC] dark:focus-visible:ring-offset-[#182B52]',
        className,
      ].join(' ')}
    >
      {heroSrc ? (
      ...
      <div className={`flex-1 flex flex-col gap-3 z-2 p-4 bg-white rounded-2xl ${heroSrc ? '-mt-6' : ''}`}>
        <div className="flex flex-col gap-2">
        <h2 className="text-[20px] md:text-[22px] font-semibold leading-snug tracking-tight text-slate-700 dark:text-[#E6EEFF] line-clamp-3">
          {a.Title}
        </h2>
        <p className="text-[15px] leading-6 text-slate-600 dark:text-slate-400 line-clamp-3">
          {a.description || '—'}
        </p></div>
        <div className="pt-3 text-[13px] font-medium text-slate-600/90 dark:text-slate-400/90 flex flex-wrap items-center gap-5">
          {a.readingTime && (
