      ...
      <article className="mx-auto max-w-3xl px-6 lg:px-0 py-12">
        <header className="mb-10">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 text-sm font-medium text-slate-700 dark:text-slate-200">
            ...
            {article.readingTime && <span className="opacity-70 leading-none flex gap-1 items-center font-medium"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
</svg>
 {article.readingTime}</span>}
            {article.displayDate && <span className="opacity-70 leading-none flex gap-1 items-center font-medium"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
  <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
</svg>
 {article.displayDate}</span>}
          </div>
        </header>
        <div className="article-body markdown-content text-lg leading-7 [&_p]:mt-6 [&_p:first-child]:mt-0 text-[#182B52] dark:text-[#E6EEFF]" id="article-content">
        ...
        <section className="mt-14">
          <div className="relative overflow-hidden rounded-2xl border border-[#E2E8F0] dark:border-[#283B63] bg-white dark:bg-[#152544] p-6 sm:p-8 shadow-sm">
            <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-[#FFDC0F]/60 blur-2xl" aria-hidden />
            <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-xl font-semibold tracking-tight text-slate-700 dark:text-white">Have thoughts about this topic?</h3>
                <p className="mt-1 text-sm leading-6 text-slate-700 dark:text-slate-300">I’d love to hear your perspective. Let’s continue the conversation on LinkedIn.</p>
              </div>
        ...
        <nav className="mt-16 flex flex-col gap-4 text-sm">
          <div className="flex flex-wrap gap-4">
            {adjacent.previous && (
              <Link href={`/articles/${adjacent.previous.slug}`} className="group inline-flex flex-col rounded-md border border-slate-200 dark:border-slate-800 p-3 hover:bg-slate-50 dark:hover:bg-slate-900/60 transition">
            ...
            {adjacent.next && (
              <Link href={`/articles/${adjacent.next.slug}`} className="group inline-flex flex-col rounded-md border border-slate-200 dark:border-slate-800 p-3 hover:bg-slate-50 dark:hover:bg-slate-900/60 transition ml-auto">
                <span className="text-xs uppercase tracking-wide text-slate-500 text-right">Next</span>
