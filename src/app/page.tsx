...
function Timeline() {
  const items: TimelineItem[] = [
    ...
    { label: "Trusted Shops", span: "2017 – now", svg: "companies/logo-trusted-shops.svg" },
    ...
    <div className="mt-32 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-0 gap-y-16">
      ...
      {items.map((it, i) => (
        <div key={i} className="relative pb-8 text-center">
          <div className={`absolute left-0 right-0 bottom-1 h-0.5 bg-[#FFBF00] ${i === items.length -1 ? ("rounded-r-full"):""}`} />
          <div className="mx-auto flex h-16 w-16 items-center justify-center ">
            {"inline" in it ? (
              <svg width="109" height="64" viewBox="0 0 109 64" xmlns="http://www.w3.org/2000/svg" className="h-16 w-aut">
              ...
              <img src={`/images/${it.svg}`} alt="" className="size-16 dark:invert-60 text-red-300" />
            )}
          </div>
        ...
        {latestArticles.length > 0 && (
          <section className="mx-auto mt-16 max-w-4xl px-4 lg:px-0">
            <div className="mb-6 flex items-end justify-between gap-4">
              <h3 className="text-lg md:text-xl font-semibold tracking-tight">Latest articles by Oliver Pitsch</h3>
              {hasMoreArticles && (
                <Link href="/articles" className="text-[14px] font-medium text-indigo-700 dark:text-indigo-300 underline">
                  View all
                </Link>
              )}
            </div>
